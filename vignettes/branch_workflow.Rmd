...
Set the access token do download from Gitlab with read privilege for the repositories in question.
```
Sys.setenv(ROCHE_GITLAB_PAT = "######")
library(staged.dependencies)
```
Then go on addins and select one of the two that come with this package.
